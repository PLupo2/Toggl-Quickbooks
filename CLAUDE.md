...
- ⏭️ **3.5 Cutover (PLANNED 2026-08-13, not yet executed — awaiting explicit go-ahead)**: full sequence below, pre-flight-verified live (not assumed) at planning time. Nothing executed yet — DNS still `plupo2.github.io`, Worker route still live, tunnel config still lacks the entry. Re-verify all of this at execution time if meaningfully later, since prod keeps running until the flip.
