...
**Status: Phases 3.1–3.4 of 7 complete and verified. Phase 3.5 steps 1-5 EXECUTED 2026-08-13 (~23:14 EDT) — DNS now points at the tunnel and the FastAPI service on the Mac Mini is what's actually serving `timesync.pltheatrical.com`. The Apps Script/Worker/Pages system is no longer live traffic. Step 6 (Philip's real Cloudflare Access click-through sign-off) — container logs + `sync_job`/`sync_log` DB rows show a real browser session (full asset load: css/js/logo/favicon, not a service-token script) at 03:21–03:29 UTC on 2026-08-13, minutes after the DNS flip, that filtered to a specific date (2026-08-10) in the dashboard UI and triggered a real sync. Job `028aba9e-ad2c-45a1-bac5-b157cbb60f32` (03:21:55–03:22:12 UTC): 11 synced, 0 failed, 11 already-synced, 0 tagging-failed — all 11 new `sync_log` rows show `status=Success` with sequential real QBO `TimeActivity` IDs (1073746180–1073746190, continuing directly after 3.4's batch). This is strong evidence step 6 already happened, but the app logs no per-request identity (`auth.py` resolves an email from the CF Access JWT but nothing persists it), so it isn't logged proof it was Philip specifically. One transient `401` on a `getSyncJobStatus` poll at 03:28:02 UTC, self-recovered 47s later with a full successful page reload — not investigated further, worth a mention if it recurs. **Ask Philip to confirm this was his click-through before marking step 6 done.**
...
- ⬜ 3.6 — pending step 6 (Philip's live sign-off) above, then rollback lever stays armed for a burn-in window → 3.7 two-week dormancy then demolition of the Apps Script/Worker/Pages system.
